...
### Example 1 — breaking (score: 100)

Contract has `email (string, required)`, `user_id (integer, required)`, and `created_at (timestamp, required)`.
Incoming schema is missing `email` and `user_id` entirely, and `created_at` has changed type to `string`.
