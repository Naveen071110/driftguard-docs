...
Every change DriftGuard detects gets a severity level. Understanding what triggers each level helps you configure contracts correctly and reduce false positives.
...
<Info>
  Custom per-column severity overrides are on the roadmap for v0.2. For now, use contract-level rules to tune sensitivity.
</Info>
