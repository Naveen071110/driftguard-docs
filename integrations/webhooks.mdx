...
In addition to Slack, you can send DriftGuard check results to any HTTP endpoint — your own backend, PagerDuty, a monitoring system, or a custom incident pipeline.
...
When a check completes with a status included in `event_types`, DriftGuard sends a POST request to your `target_url`:
...
Every webhook request includes a signature header so you can verify that it came from DriftGuard:
...
1. Concatenate the timestamp (`t`) and raw request body with a `.`: `t.body`
2. Compute an HMAC SHA256 using your webhook secret
3. Compare the hex digest to `v1` in constant time

Webhook secrets are generated per webhook and visible once when you create them in the dashboard.

<Warning>
  Always validate the signature and timestamp before trusting any webhook request. Reject requests older than 5 minutes.
</Warning>
...
<Card title="API Reference" icon="code" href="/api-reference/webhooks/create">
  See the full webhook API reference.
</Card>
